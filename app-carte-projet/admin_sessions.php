            ...
            <form method="POST">
                <div class="mb-4">
                    ...
                    <input type="password" name="admin_password" required
                           class="w-full p-3 border rounded focus:ring-2 focus:ring-purple-500">
                </div>
                <button type="submit" class="w-full bg-purple-700 text-white p-3 rounded hover:bg-purple-800">
                    Connexion
                </button>
            </form>
            <div class="mt-4 text-center">
                <a href="index.php" class="text-purple-600 hover:underline">Retour</a>
            </div>
        </div>
    </body>
    </html>
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'delete_session':
                ...
                $stmt = $db->prepare("DELETE FROM cartes_projet WHERE session_id = ?");
...
foreach ($sessions as &$session) {
    ...
    $stmt = $db->prepare("SELECT COUNT(*) FROM cartes_projet WHERE session_id = ? AND is_submitted = 1");
    $stmt->execute([$session['id']]);
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Administration - Carte d'identite du projet</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
...
<body class="p-4">
    <div class="max-w-6xl mx-auto">
        <div class="bg-purple-900 text-white p-4 rounded-t-lg flex justify-between items-center">
            <h1 class="text-xl font-bold">Administration - Carte d'identite du projet</h1>
            <div class="flex gap-4">
                <a href="index.php" class="bg-purple-700 hover:bg-purple-600 px-4 py-2 rounded">Accueil</a>
                            ...
                            <tbody class="divide-y">
                                <?php foreach ($sessions as $session): ?>
                                    <tr class="hover:bg-gray-50">
                                        <td class="p-4">
                                            <span class="font-mono font-bold text-purple-700"><?= sanitize($session['code']) ?></span>
                                        </td>
