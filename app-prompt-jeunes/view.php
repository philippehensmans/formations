...
// Recuperer tous les exercices du participant
$stmt = $db->prepare("SELECT * FROM travaux WHERE user_id = ? AND session_id = ? ORDER BY exercice_num ASC");
$stmt->execute([$participant['user_id'], $participant['session_id']]);
$allExercices = $stmt->fetchAll();

// Determiner l'exercice a afficher
$currentExerciceNum = isset($_GET['ex']) ? (int)$_GET['ex'] : null;
$travail = null;

if ($currentExerciceNum !== null) {
    foreach ($allExercices as $ex) {
        if ($ex['exercice_num'] == $currentExerciceNum) {
            $travail = $ex;
            break;
        }
    }
} elseif (!empty($allExercices)) {
    $travail = $allExercices[0];
    $currentExerciceNum = $travail['exercice_num'];
}
...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="bg-pink-50 border-b border-pink-200 no-print">
        <div class="max-w-5xl mx-auto px-4 py-2 flex items-center gap-2 flex-wrap">
            <span class="text-sm text-pink-700 font-medium">Exercices :</span>
            <?php foreach ($allExercices as $ex):
                $exCas = $casLabels[$ex['cas_choisi']] ?? null;
            ?>
                <a href="view.php?id=<?= $participantId ?>&ex=<?= $ex['exercice_num'] ?>"
                   class="px-3 py-1 rounded-full text-sm <?= $ex['exercice_num'] == $currentExerciceNum ? 'bg-pink-500 text-white' : 'bg-white text-pink-700 hover:bg-pink-100' ?> border border-pink-300 flex items-center gap-1">
                    ...
                    <?php if ($ex['cas_choisi']): ?>
                        <span class="text-xs opacity-75">(<?= $exCas ? substr($exCas['label'], 0, 10) : $ex['cas_choisi'] ?>)</span>
                    <?php endif; ?>
                    <?php if ($ex['is_shared']): ?>
