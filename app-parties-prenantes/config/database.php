 ...
 * Application de formation - Cartographie des Parties Prenantes
 */
...
function getSessionStats($sessionId) {
    ...
    $stmt = $db->prepare("
        SELECT
            ...
            COUNT(CASE WHEN c.is_submitted = 1 THEN 1 END) as submitted_count,
            COALESCE(AVG(c.completion_percent), 0) as avg_completion
        FROM participants p
...
function calculateCompletion($data) {
    ...
    // Titre projet (20 points)
    $total += 20;
    if (!empty($data['titre_projet'])) $filled += 20;

    // Stakeholders (60 points si au moins 3)
    $total += 60;
    $stakeholders = $data['stakeholders_data'] ?? [];
    if (is_array($stakeholders)) {
        $count = count($stakeholders);
        if ($count >= 3) $filled += 60;
        elseif ($count >= 1) $filled += 30;
    }

    // Notes (20 points)
    $total += 20;
    if (!empty($data['notes'])) $filled += 20;
