    ...
    <body class="bg-gray-100 min-h-screen flex items-center justify-center">
        <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
            ...
            <form method="POST">
                ...
                <button type="submit" class="w-full bg-blue-900 text-white p-3 rounded hover:bg-blue-800">
                    Connexion
                </button>
            </form>
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'delete_session':
                ...
                $stmt = $db->prepare("DELETE FROM retrospectives WHERE session_id = ?");
...
foreach ($sessions as &$session) {
    ...
    $stmt = $db->prepare("SELECT COUNT(*) FROM retrospectives WHERE session_id = ? AND is_submitted = 1");
    $stmt->execute([$session['id']]);
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Administration - Stop Start Continue</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 min-h-screen">
    <div class="bg-blue-900 text-white p-4">
        <div class="max-w-6xl mx-auto flex justify-between items-center">
            <h1 class="text-xl font-bold">Administration - Stop Start Continue</h1>
            <div class="flex gap-4">
                <a href="index.php" class="bg-blue-700 hover:bg-blue-600 px-4 py-2 rounded">Accueil</a>
