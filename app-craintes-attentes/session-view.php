<?php
/**
 * Vue globale de session - Craintes et Attentes
 * Affiche tous les avis de tous les participants avec filtrage par categorie
 */
...
// Recuperer les avis de la session
if ($showAll) {
...
<head>
    ...
    <title>Craintes & Attentes - Vue Session - <?= h($session['nom']) ?></title>
...
<body class="bg-gradient-to-br from-teal-50 to-emerald-100 min-h-screen">
    <!-- Header -->
    <header class="bg-gradient-to-r from-teal-600 to-emerald-700 text-white shadow-lg no-print sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 py-4">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold">⚖️ Craintes & Attentes</h1>
                    <p class="text-teal-200 text-sm"><?= h($session['nom']) ?> - <?= $session['code'] ?></p>
                </div>
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
            <div class="flex justify-between items-start">
                ...
                <button onclick="openSujetModal()" class="no-print ml-4 px-3 py-1 bg-teal-100 text-teal-700 hover:bg-teal-200 rounded text-sm">
                    Modifier
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div class="bg-white rounded-xl shadow p-4 text-center">
                <div class="text-3xl font-bold text-teal-600"><?= $totalAvis ?></div>
            ...
            <div class="bg-white rounded-xl shadow p-4 text-center">
                <div class="text-3xl font-bold text-red-600"><?= $stats['craintes'] ?? 0 ?></div>
                <div class="text-gray-500 text-sm">😟 Craintes</div>
            </div>
            <div class="bg-white rounded-xl shadow p-4 text-center">
                <div class="text-3xl font-bold text-green-600"><?= $stats['attentes'] ?? 0 ?></div>
                <div class="text-gray-500 text-sm">🌟 Attentes</div>
            </div>
        </div>

        <!-- Repartition visuelle -->
        <div class="bg-white rounded-xl shadow p-6 mb-8">
            <h2 class="text-lg font-bold text-gray-800 mb-4">Repartition Craintes / Attentes</h2>
            <?php
            $craintesCount = $stats['craintes'] ?? 0;
            $attentesCount = $stats['attentes'] ?? 0;
            $craintesPercent = $totalAvis > 0 ? round(($craintesCount / $totalAvis) * 100) : 0;
            $attentesPercent = $totalAvis > 0 ? round(($attentesCount / $totalAvis) * 100) : 0;
            ?>
            <div class="flex gap-6 items-center">
                <div class="flex-1">
                    <div class="flex justify-between mb-2">
                        <span class="text-red-600 font-bold">😟 Craintes (<?= $craintesCount ?>)</span>
                        <span class="text-red-600 font-bold"><?= $craintesPercent ?>%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-6">
                        <div class="bg-red-400 h-6 rounded-full transition-all duration-500" style="width: <?= $craintesPercent ?>%"></div>
                    </div>
                </div>
                <div class="flex-1">
                    <div class="flex justify-between mb-2">
                        <span class="text-green-600 font-bold">🌟 Attentes (<?= $attentesCount ?>)</span>
                        <span class="text-green-600 font-bold"><?= $attentesPercent ?>%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-6">
                        <div class="bg-green-400 h-6 rounded-full transition-all duration-500" style="width: <?= $attentesPercent ?>%"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow p-4 mb-6 no-print">
            <div class="flex flex-wrap gap-4 items-center">
                <div class="font-medium text-gray-700">Filtrer:</div>
                <button onclick="filterByChapeau('all')" id="filter-all"
                        class="category-filter px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm font-medium active ring-teal-500">
                    Tous (<?= $totalAvis ?>)
                </button>
                ...
                <button onclick="filterByChapeau('<?= $key ?>')" id="filter-<?= $key ?>"
                        class="category-filter px-4 py-2 <?= $chapeau['bg'] ?> <?= $chapeau['text'] ?> hover:opacity-80 rounded-lg text-sm font-medium border <?= $chapeau['border'] ?>">
                    <?= $chapeau['icon'] ?> <?= $chapeau['nom'] ?> (<?= $count ?>)
                </button>
                <?php endforeach; ?>
            </div>
            <div class="mt-4 flex gap-4 items-center">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    ...
                    <span class="text-sm">Colonnes par categorie</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
        ...
        <div id="gridView" class="grid md:grid-cols-2 gap-6">
            <?php foreach ($chapeaux as $key => $chapeau): ?>
            <div class="bg-white rounded-xl shadow-lg overflow-hidden chapeau-section" data-chapeau="<?= $key ?>">
                <div class="<?= $chapeau['bg'] ?> <?= $chapeau['text'] ?> p-4 border-b-4 <?= $chapeau['border'] ?>">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="bg-white/30 px-3 py-1 rounded text-sm font-bold"><?= count($avisByChapeau[$key]) ?></span>
                    </div>
                </div>
        ...
        <div id="participantView" class="hidden space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden participant-section">
                <div class="bg-gradient-to-r from-teal-500 to-emerald-500 text-white p-4">
                    <div class="flex justify-between items-center">
                        <div>
                            <span class="font-bold"><?= h($data['user']['prenom']) ?> <?= h($data['user']['nom']) ?></span>
                            <?php if (!empty($data['user']['organisation'])): ?>
                            <span class="text-teal-200 text-sm ml-2">(<?= h($data['user']['organisation']) ?>)</span>
                            <?php endif; ?>
                        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col">
            <div class="flex justify-between items-center p-6 border-b">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-xl font-bold text-gray-800">Synthese IA - Craintes & Attentes</h3>
                </div>
            ...
            <div class="p-4 border-t flex justify-end gap-3">
                ...
                <button onclick="printAISummary()" class="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg flex items-center gap-2">
    ...
    <div id="sujetModal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4 no-print">
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button onclick="saveSujet()" class="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg">
                    Enregistrer
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function filterByChapeau(chapeau) {
            ...
            document.querySelectorAll('.category-filter').forEach(btn => {
                btn.classList.remove('active', 'ring-4', 'ring-teal-500');
            });
            document.getElementById('filter-' + chapeau).classList.add('active', 'ring-4', 'ring-teal-500');
        ...
        async function generateAISummary() {
            ...
            document.getElementById('aiContent').innerHTML = `
                <div class="flex flex-col items-center justify-center py-16">
                    ...
                    <p class="text-gray-600 font-medium">Analyse des contributions en cours...</p>
                    <p class="text-gray-400 text-sm mt-2">Claude analyse les craintes et attentes de tous les participants</p>
                </div>
            `;
        ...
        function printAISummary() {
            ...
            w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Synthese IA - <?= h($session['nom']) ?></title>
                ...
                <body><h1 class="text-2xl font-bold mb-2">Synthese IA - Craintes & Attentes</h1>
                <p class="text-gray-500 mb-6"><?= h($session['nom']) ?> | Session <?= $session['code'] ?> | Genere le ${new Date().toLocaleDateString('fr-FR')} a ${new Date().toLocaleTimeString('fr-FR')}</p>
                ${content}</body></html>`);
            w.document.close();
